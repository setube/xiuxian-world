...
export const SECTS: Record<string, Sect> = {
  ...
  luoyun: {
    ...
    name: '落云宗',
    shortName: '落云',
    description: '守护神木的古老宗门，以灵眼之树闻名。',
    lore: '落云宗建于上古灵眼之树旁，千年来守护这棵神秘古树。宗门弟子共同培育灵树，收获珍贵的灵眼之液，可升级灵根、催熟灵草。',
    alignment: 'righteous',
    color: '#22C55E',
    icon: 'TreePine',
    location: '落云峰',
    features: [
      { type: 'cultivation_boost', name: '灵眼之树', description: '全宗共同培育神树，获取珍稀资源', value: 1 },
      { type: 'cultivation_boost', name: '木灵亲和', description: '木灵根弟子浇灌贡献翻倍', value: 2.0 },
      { type: 'alchemy_boost', name: '灵眼之液', description: '可升级灵根或催熟灵草', value: 1 }
    ],
    requirements: { minRealm: 1 },
    baseSalary: 45,
    specialSkills: ['spirit_tree_art', 'wood_affinity'],
    dailyBonus: { type: 'watering', description: '每日浇灌灵树：贡献成熟度和获取贡献值' }
  }
}
...
// 计算每日俸禄
export const calculateDailySalary = (sect: Sect, rank: SectRank): number => {
  const rankInfo = getSectRankInfo(rank)
  if (!rankInfo) return 0

  let salary = sect.baseSalary * rankInfo.salaryMultiplier

  // 星宫特殊加成
  if (sect.id === 'xinggong') {
    const salaryFeature = sect.features.find(f => f.type === 'salary')
...
export const COMMON_TREASURY_ITEMS: TreasuryItem[] = [
  ...
  },
  {
    ...
    type: 'pill',
    quality: 'legendary',
    description: '蕴含护道之力的绝世灵丹，可在神魂陨落时自动消耗，免除境界跌落、修为清零、物品掉落的惩罚',
    effect: '神魂陨落时自动消耗，保护修士免受惨重损失（仍会进入道心破碎状态）',
    contributionCost: 10000,
    rankRequired: 'inner',
    realmRequired: 2,
    limitPerDay: 1,
    stock: -1
...
export interface SectBounty {
  ...
  refreshTime?: number // 刷新时间（毫秒），仅daily/weekly
}
