...
export const CONSORT_CONFIG = {
  ...
  greetingAffectionGain: 5,
  giftCostBase: 100,
  giftAffectionGain: 3,
  spiritFeedbackCost: 500,
  spiritFeedbackBonus: 30,
  spiritFeedbackDurationMs: 3600000,
  divinationCost: 200,
  maxAffection: 100,
  passiveRedSleeveBonus: 10
}
...
export interface ConsortStatus {
  ...
  spiritFeedbackExpiresIn: number
...
export interface ObservatoryStatus {
  disks: DiskStatus[]
  unlockedDisks: number
  maxDisks: number
  canExpand: boolean
  expansionCost: number
  consortAssignment: {
    consortId: string | null
    consortName: string | null
...
export interface StargazeStatus {
  canStargaze: boolean
  stargazeCooldownMs: number
  currentResult: StargazeResult | null
  resultExpiresAt: number | null
  canChangeFate: boolean
  changeFateCooldownMs: number
  freeChangeFateCount: number
}
