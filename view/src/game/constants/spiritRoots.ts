...
export interface SpiritRoot {
  ...
  name: string
  element: string // 五行属性
  elements: string[] // 五行属性数组（用于相克判断）
  color: string // 显示颜色
  rarity: 'common' | 'rare' | 'epic' | 'legendary' | 'mythic'
  rootType: RootType // 灵根类型：天灵根/异灵根/真灵根/伪灵根/废灵根
  description: string
  cultivationBonus: number // 修炼速度加成 (%)
  combatPowerBonus: number // 基础战力加成 (%)
  attributes: {
    attack?: number
    defense?: number
    speed?: number
...
export const ELEMENT_COUNTER: Record<string, string[]> = {
  ...
  水: ['火'], // 水克火
  火: ['金'], // 火克金
  雷: ['金', '木', '水', '火', '土'], // 雷御万法，克制所有基础五行
  冰: ['火'], // 冰克火
  风: [] // 风无特殊克制
}

// 相克伤害加成
export const COUNTER_DAMAGE_BONUS = 0.15 // 15%

// 检查是否能触发相克（只有天灵根和异灵根）
export const canTriggerCounter = (root: SpiritRoot): boolean => {
  return root.rootType === 'heavenly' || root.rootType === 'variant'
}

// 获取相克伤害加成
export const getCounterBonus = (attacker: SpiritRoot, defender: SpiritRoot): number => {
...
export const RARITY_CONFIG = {
  ...
  mythic: { label: '天品', color: '#f0e6c8', weight: 2 }
}

// 根据权重随机获取灵根
export const randomSpiritRoot = (): SpiritRoot => {
  const roots = Object.values(SPIRIT_ROOTS)
  const totalWeight = roots.reduce((sum, root) => {
    return sum + RARITY_CONFIG[root.rarity].weight
  }, 0)

  let random = Math.random() * totalWeight
  for (const root of roots) {
    random -= RARITY_CONFIG[root.rarity].weight
    if (random <= 0) {
      return root
    }
  }
...
const DAO_NAME_SUFFIXES = ['子', '真人', '道人', '仙', '君', '尊', '帝', '圣', '阳', '阴', '虚', '灵', '玄', '微', '明', '空']

const DAO_NAME_MIDDLES = ['一', '无', '太', '玄', '真', '清', '净', '明', '虚', '空', '元', '始', '上', '大', '至', '极']

export const generateDaoName = (): string => {
  const prefix = DAO_NAME_PREFIXES[Math.floor(Math.random() * DAO_NAME_PREFIXES.length)]!
  const middle = DAO_NAME_MIDDLES[Math.floor(Math.random() * DAO_NAME_MIDDLES.length)]!
